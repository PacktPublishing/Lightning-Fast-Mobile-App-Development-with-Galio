...
export default function CommerceCard(props) {
    ...
    return (
        <Block row style={styles.card}>
            <Image style={styles.item} source={{ uri: "https://picsum.photos/100" }}/>
            <Block style={{ marginLeft: 16 }} flex space="around">
                <Text>{props.itemName}</Text>
                <Block row space="between">
                    <Text>$ {price}</Text>
                    <Block row space="between">
                        <TouchableOpacity style={styles.buttons} onPress={() => quantityMath("minus")}>
                            <Icon name="minus" family="entypo"/>
                        </TouchableOpacity>
                        <Text>{quantity}</Text>
                        <TouchableOpacity style={styles.buttons} onPress={() => quantityMath("plus")}>
